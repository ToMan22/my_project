...
class User extends \TCG\Voyager\Models\User implements MustVerifyEmail
{
    ...
    {
        return $this->email_verified_at && $this->birthdate && ($this->verification && $this->verification->status == 'verified');
    }

    public function isMemberShipCreator()
    {
        return MembershipCreator::where('creator_id', $this->id)->where('status', 'active')->exists();
    }

    public function isActiveMemberShipUser()
    {
        return Membership::where('user_id', $this->id)->where('status', 'active')->exists();
    }

    public function wasAlreadyMemberShipCreator()
    {
        return MembershipCreator::where('creator_id', $this->id)->where('status', 'inactive')->exists();
    }
}
