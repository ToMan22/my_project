...
class SettingsController extends Controller
{
    ...
    public $availableSettings = [
        ...
        'membership' => ['heading' => '💡 Join the Exclusive Membership Program!', 'icon' => 'shield-checkmark'],
    ...
    public function index(Request $request)
    {
        ...
        switch ($request->route('type')) {
            ...
            case 'rates':
                ...
                break;
            case 'membership':
                $data['allcreators'] = MembershipCreator::where('status', 'active')->get();
                break;
        }

        $creatorCategorise = CreatorCategory::latest()->get();
        $data['creatorCategorise'] = $creatorCategorise;
    ...
    public function saveVerifyRequest(Request $request)
    {
        if ($request->session()->get('verifyAssets')) {
            ...
            if ($request->has('creator_categories') && is_array($request->get('creator_categories'))) {
                // For multiple categories
                $categoryIds = $request->get('creator_categories');

                // Link each attachment to each category
                info("Loop for User");
                foreach ($categoryIds as $categoryId) {
                    info("Trying to insert: category_id = $categoryId");
                    try {
                        DB::table('creator_category')->updateOrInsert(
                            ['user_id' => auth()->id(), 'creator_category_id' => $categoryId],
                            ['created_at' => now(), 'updated_at' => now()]
                        );
                        info("Saved User");
                    } catch (\Exception $e) {
                        info("Error saving User: " . $e->getMessage());
                    }
                }
            }
