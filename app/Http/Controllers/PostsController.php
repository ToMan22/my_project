...
class PostsController extends Controller
{
    ...
    public function create()
    {
        ...
        $contentCategorise = ContentCategory::latest()->get();

        return view('pages.create', ['contentCategorise' => $contentCategorise]);
    }
    ...
    {
        info("All Data: " . print_r($request->all(), true));
        try {
            ...
            if ($postID) {
                ...
                // Now handle categories for these attachments
                info("checking attachments");
                if ($request->has('content_categories') && is_array($request->get('content_categories'))) {
                    // For multiple categories
                    $categoryIds = $request->get('content_categories');

                    // Link each attachment to each category
                    info("Loop for attachments");
                    foreach ($attachments as $attachmentId) {
                        info("Attachments Found");
                        foreach ($categoryIds as $categoryId) {
                            info("Trying to insert: attachment_id = $attachmentId, content_category_id = $categoryId");
                            try {
                                DB::table('attachment_category')->updateOrInsert(
                                    ['attachment_id' => $attachmentId, 'content_category_id' => $categoryId],
                                    ['created_at' => now(), 'updated_at' => now()]
                                );
                                info("Saved Attachment");
                            } catch (\Exception $e) {
                                info("Error saving attachment: " . $e->getMessage());
                            }
                        }
                    }
                }
